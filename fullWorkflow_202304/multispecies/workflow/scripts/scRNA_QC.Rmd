...
text(
  x = 0.1, y = 0.815, paste0("Cells removed by MiQC: ", length(which(so_prefilter$miQC.keep == "discard"))),
...
for (res in resolutionString) {
  ...
  clusterPlot <- DimPlot(so,
    group.by = paste0("SLM_res_sample.", resMod), label = T, repel = T,
...
for (res in resolutionString) {
  ...
  cat("  \n##### Clustering at Resolution ", res, " \n")
  clusterPlot <- DimPlot(so,
    group.by = paste0("Leiden_res_sample.", resMod), label = T, repel = T,
