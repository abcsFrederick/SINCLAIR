...
h5_fileName <- paste0("cellRangerOut/", sample, "/outs/filtered_feature_bc_matrix.h5")
...
seuratClustering <- function(so) {
  ...
  for (res in resolution) {
    so <- FindClusters(so, dims = 1:npcs, print.output = 0, resolution = res, algorithm = 3)
    so[[paste0("SLM_res_", res)]] <- so$seurat_clusters
    # so <- FindClusters(so,dims = 1:npcs, print.output = 0, resolution = res, algorithm = 4)
  }
...
so <- CreateSeuratObject(counts = h5)
so$Sample <- sample
...
colorList <- hue_pal()(3)
venn.diagram(
  x = list(cellsToRemove.Feature, cellsToRemove.Count, cellsToRemove.Mito),
...
vln_nFeature_preFilter <- ggplot(so@meta.data, aes(x = orig.ident, y = log2(as.numeric(as.character(nFeature_RNA))), fill = orig.ident))
...
vln_nCount_preFilter <- ggplot(so@meta.data, aes(x = orig.ident, y = log2(as.numeric(as.character(nCount_RNA))), fill = orig.ident))
...
vln_pctMito_preFilter <- ggplot(so@meta.data, aes(x = orig.ident, y = log2(as.numeric(as.character(percent.mt))), fill = orig.ident))
vln_pctMito_preFilter <- vln_pctMito_preFilter + geom_violin(trim = F) + scale_fill_manual(values = hue_pal()(3)[3], labels = c("Percent Mito")) +
  geom_boxplot(width = 0.1, fill = "white") +
  scale_x_discrete(labels = NULL) +
  theme_classic() + geom_hline(yintercept = log2(mt_out), linetype = "dashed") + labs(x = paste(sample, "Pre-filter"), y = "Log2(Percent Mito)") +
  labs(fill = paste(sample, "Pre-filter"), title = "Pre-filter Percent Mito")


# Generate initial filter data
so <- subset(so, cells = colnames(so)[which(log2(so$nFeature_RNA) > nFeature_out & log2(so$nCount_RNA) > nFeature_out & so$percent.mt < mt_out)])
...
vln_nFeature_postFilter <- vln_nFeature_postFilter + geom_violin(trim = F) + scale_fill_manual(values = hue_pal()(3)[1], labels = c("nFeature")) +
  geom_boxplot(width = 0.1, fill = "white") +
  scale_x_discrete(labels = NULL) +
  theme_classic() + geom_hline(yintercept = log2(nFeature_out), linetype = "dashed") + labs(x = paste(sample, "Post-filter"), y = "Log2(nFeature_RNA)") +
...
vln_nCount_postFilter <- vln_nCount_postFilter + geom_violin(trim = F) + scale_fill_manual(values = hue_pal()(3)[2], labels = c("nCount")) +
  geom_boxplot(width = 0.1, fill = "white") +
  scale_x_discrete(labels = NULL) +
  theme_classic() + geom_hline(yintercept = nCount_out, linetype = "dashed") + labs(x = paste(sample, "Post-filter"), y = "Log2(nCount_RNA)") +
...
vln_pctMito_postFilter <- vln_pctMito_postFilter + geom_violin(trim = F) + scale_fill_manual(values = hue_pal()(3)[3], labels = c("Percent Mito")) +
  geom_boxplot(width = 0.1, fill = "white") +
  scale_x_discrete(labels = NULL) +
  theme_classic() + geom_hline(yintercept = log2(mt_out), linetype = "dashed") + labs(x = paste(sample, "Post-filter"), y = "Log2(Percent Mito)") +
...
text(
  x = 0.05, y = 0.95, paste0("QC Metrics for ", sample, ":"),
  cex = 1.5, col = "black", family = "serif", font = 2, adj = 0
)

text(
  x = 0.075, y = 0.9, paste0("Cells before filtering: ", prefilter_count),
  cex = 1.25, col = "black", family = "serif", font = 3, adj = 0
)
text(
  x = 0.075, y = 0.865, paste0("Cells after filtering: ", ncol(so)),
  cex = 1.25, col = "black", family = "serif", font = 3, adj = 0
)

text(
  x = 0.1, y = 0.815, paste0("Cells removed for nFeature (number of genes): ", length(cellsToRemove.Feature)),
  cex = 1, col = "dimgray", family = "serif", font = 1, adj = 0
)
text(
  x = 0.1, y = 0.785, paste0("Cells removed for nCount (number of reads): ", length(cellsToRemove.Count)),
  cex = 1, col = "dimgray", family = "serif", font = 1, adj = 0
)
text(
  x = 0.1, y = 0.755, paste0("Cells removed for mitochondrial percentage: ", length(cellsToRemove.Mito)),
  cex = 1, col = "dimgray", family = "serif", font = 1, adj = 0
)
text(
  x = 0.1, y = 0.725, paste0("Cells removed as doublets: ", doubletCount),
  cex = 1, col = "dimgray", family = "serif", font = 1, adj = 0
)

text(
  x = 0.1, y = 0.68, paste0("Number of principal components used: ", so@misc$npcs),
  cex = 1, col = "dimgray", family = "serif", font = 1, adj = 0
)
...
for (algorithm in c("SLM", "Leiden")) {
  ...
  for (i in 1:length(resolution)) {
    Idents(so) <- paste0(algorithm, "_res_", resolution[i])
    ...
    print(plot(sil,
      col = palette[as.factor(clusters[order(as.numeric(as.character(clusters)), decreasing = F)])],
...
p2 <- ggdraw() + draw_image(list.files(path = imageDir, pattern = "Venn.*png$", full.names = T), scale = 1)
...
```{r nFeature, fig.width=15, fig.height=9, echo=FALSE, results="asis"}
p1 <- ggdraw(vln_nFeature_preFilter)
p2 <- ggdraw(vln_nFeature_postFilter)
plot_grid(p1, p2, ncol = 2)
```

#### nCount (Number of reads)
```{r nCount, fig.width=15, fig.height=9, echo=FALSE, results="asis"}
p1 <- ggdraw(vln_nCount_preFilter)
p2 <- ggdraw(vln_nCount_postFilter)
plot_grid(p1, p2, ncol = 2)
```

#### Mitochondrial percentage
```{r mitoPct, fig.width=15, fig.height=9, echo=FALSE, results="asis"}
p1 <- ggdraw(vln_pctMito_preFilter)
p2 <- ggdraw(vln_pctMito_postFilter)
plot_grid(p1, p2, ncol = 2)
```

#### Doublets
```{r Doublets, fig.width=6, fig.height=6}
p1 <- ggdraw(doubletPlot)
plot_grid(p1)
```
...
for (res in resolutionString) {
  ...
  clusterPlot <- DimPlot(so,
    group.by = paste0("SLM_res_", resMod), label = T, repel = T,
    order = sort(as.numeric(as.character(unique(so[[paste0("SLM_res_", resMod)]][, 1]))), decreasing = T)
  ) +
...
plot(resSil_SLM,
  lty = 3, pch = 1, type = "b", lwd = 2, cex = 3,
...
```{r clusteringResults_Leiden,fig.width=15,fig.height=9,echo=FALSE,results="asis",eval=FALSE}
for (res in resolutionString) {
  resMod <- as.numeric(res)
  fileName <- paste0(imageDir, "/silhouetteResolution_Leiden_", resolution[i], ".pdf")
  cat("  \n##### Clustering at Resolution ", res, " \n")
  clusterPlot <- DimPlot(so,
    group.by = paste0("Leiden_res_", resMod), label = T, repel = T,
    order = sort(as.numeric(as.character(unique(so[[paste0("Leiden", resMod)]][, 1]))), decreasing = T)
  ) +
...
plot(resSil_Leiden,
  lty = 3, pch = 1, type = "b", lwd = 2, cex = 3,
  main = "Average silhouette score vs clustering resolution", xlab = "Resolution", ylab = "Average Silhouette Score",
  xlim = c(min(resSil_Leiden[, 1]) - 0.1, max(resSil_Leiden[, 1]) + 0.1),
  ylim = c(round(min(resSil_Leiden[, 2]) - 0.005, digits = 3), round(max(resSil_Leiden[, 2]) + 0.005, digits = 3))
)
