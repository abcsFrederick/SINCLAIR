...
text(x=0.1, y=0.785,paste0("Cells removed for nCount (number of reads): ",length(cellsToRemove.Count)),
		 cex = 1, col="dimgray",family="serif", font = 1, adj =0)
...
for (res in resolutionString){
  ...
  clusterPlot = DimPlot(so, group.by=paste0("SLM_res_",resMod),label=T,repel=T,
...
for (res in resolutionString){
  ...
  cat("  \n##### Clustering at Resolution ", res," \n")
  clusterPlot = DimPlot(so, group.by=paste0("Leiden_res_",resMod),label=T,repel=T,
