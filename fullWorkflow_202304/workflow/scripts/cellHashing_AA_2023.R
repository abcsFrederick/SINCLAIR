...
hashingTable <- read.delim("hashing.tab", header = F, stringsAsFactors = F)
colnames(hashingTable) <- c("Sample", "HTO", "name")

h5 <- Read10X_h5("filtered_feature_bc_matrix.h5")
...
for (n in 1:length(so.list)) {
  ncells <- append(ncells, ncol(so.list[[n]]))
}
...
for (n in 1:length(so.list)) {
  saveRDS(so.list[[n]], paste0("x_", names(so.list)[n]), ".rds")
}
