...
nCount_RNA_max = params$nCount_RNA_max
nCount_RNA_min = params$nCount_RNA_min
nFeature_RNA_max = params$nFeature_RNA_max
nFeature_RNA_min = params$nFeature_RNA_min
percent_mt_max = params$percent_mt_max
percent_mt_min = params$percent_mt_min
...
# perform QC depenedent on user choice
if(qc_filtering == "manual") {
  so_qc_select =  subset(so_qc,
  nCount_RNA_max > nCount_RNA < nCount_RNA_min & 
  nFeature_RNA_max > nFeature_RNA < nFeature_RNA_min & 
  percent_mt_max > percent.mt < percent_mt_min)
  ...
  nCount_RNA > nFeature_out & 
  percent.mt < mt_out)
}

# Process data
so_processed = MAIN_PROCESS_SO(so_filt, species, ncps_val)

# Label data
so_labeled = MAIN_LABEL_SO(so_processed,species)

# ID doublets
so_doublet = PROCESS_DOUBLETS(so_labeled, run_doublet_finder)
```
